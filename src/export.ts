 ...
 */
export async function spreadToPng(question: string, hand: Drawn[], faces: SVGSVGElement[], theme: Theme): Promise<Blob> {
  ...
  const cards = hand
    .map((h, k) => {
      ...
      return `<text x="${x + CW / 2}" y="${y0 - 14}" font-size="13" letter-spacing="2" fill="${theme.nav}" text-anchor="middle">${POSITIONS[k]}</text>
        <g transform="translate(${x},${y0}) scale(${CW / 120})">${inner}</g>`
    })
    .join('')
  const names = hand.map((h, k) => `${POSITIONS[k]} ${h.card.zh}${h.reversed ? '（逆位）' : ''}`).join(' · ')
  const svg = `<svg xmlns="http://www.w3.org/2000/svg" width="${W}" height="${H}" viewBox="0 0 ${W} ${H}" font-family="${FONT}">
    ...
    <text x="48" y="108" font-size="22" font-style="italic" fill="${theme.copy}">"${esc(question)}"</text>
    ${cards}
